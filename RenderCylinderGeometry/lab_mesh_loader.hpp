...
namespace lab{
	...
	void _loadObjFile(const std::string &filename, std::vector<VertexFormat> &vertices, std::vector<unsigned int> &indices){
		...
		std::vector<glm::vec3> positions;		positions.reserve(100000);
		std::vector<glm::vec3> normals;			normals.reserve(100000);
		std::vector<glm::vec2> texcoords;		texcoords.reserve(100000);
